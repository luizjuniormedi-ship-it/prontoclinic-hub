...
grep -Fq 'manifest.migrationVersion !== version' "$helper"
grep -Fq 'manifest.migrationName !== name' "$helper"
! grep -Fq "grep -Fq '20260804033225_secure_companies_units_admin_contract'" "$helper"
...
grep -Fq 'ALTER TABLE public.companies FORCE ROW LEVEL SECURITY' "$rollback"
grep -Fq 'ALTER TABLE public.units FORCE ROW LEVEL SECURITY' "$rollback"
! grep -Fq 'NO FORCE ROW LEVEL SECURITY' "$rollback"
