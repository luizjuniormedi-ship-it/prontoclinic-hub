...
import pg from 'pg';
import { IMAGING_RPC_ROLE_POLICY, isRpcRoleAllowed } from './scripts/rpc-role-policy.mjs';
const { Pool } = pg;
...
async function queryWithTenantContext(companyId, text, values = []) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    await client.query(
      `SELECT set_config('request.jwt.claim.company_id', $1, true)`,
      [companyId || ''],
    );
...
async function verifyPassword(email, password) {
  ...
  try {
    res = await pool.query(
      `SELECT u.id, u.email, u.encrypted_password, u.email_confirmed_at, u.raw_app_meta_data, u.raw_user_meta_data,
              p.lg_ativo, s.must_change_password, s.password_expires_at, s.mfa_required
         FROM auth.users u
         JOIN public.user_profiles p ON p.id = u.id
         LEFT JOIN public.auth_account_security s ON s.user_id = u.id
        WHERE u.email = $1
    ...
    res = await pool.query(
      `SELECT u.id, u.email, u.encrypted_password, u.email_confirmed_at, u.raw_app_meta_data, u.raw_user_meta_data,
         ...
         FROM auth.users u
         JOIN public.user_profiles p ON p.id = u.id
        WHERE u.email = $1 AND p.lg_ativo IS TRUE`,
      [email.toLowerCase().trim()]
    );
  }
  ...
  if (!check.rows[0]?.valid) {
    await updateAuthSecurity(user.id, false);
    return null;
  }
  await updateAuthSecurity(user.id, true);
  return user;
}
...
async function updateAuthSecurity(userId, success) {
  try {
    await pool.query(
...
async function authorizeRpc(profile, functionName) {
  ...
  const role = (profile.role_name || '').toLowerCase();
  if (required.allowedRoles) {
    if (!isRpcRoleAllowed(required.allowedRoles, role)) {
      return { ok: false, reason: `role '${role}' nao autorizada para RPC '${functionName}'` };
    }
    // Estes dois RPCs fazem as verificacoes de tenant, profissional ativo,
    // vinculo ao atendimento/laudo e estado transacional no proprio banco.
    return { ok: true };
  }
  if (role === 'admin') {
    return { ok: true };
  ...
  try {
    ...
    if (path === '/auth/v1/user' && req.method === 'PUT') {
      ...
      if (!isStrongPassword(password)) {
        return json(res, {
          error: 'weak_password',
          error_description: 'Password must contain at least 10 characters, uppercase, lowercase, number and symbol',
        }, 422);
      }
      ...
      try {
        await pool.query(
          `UPDATE public.auth_account_security
    ...
    if (path.startsWith('/rest/v1/rpc/') && req.method === 'POST') {
      ...
      try {
        ...
        await client.query(
          ...
          [payload.sub, profile.company_id || '', JSON.stringify(payload)],
        );
    ...
    if (path.startsWith('/rest/v1/')) {
      ...
      if (req.method === 'GET') {
        ...
        try {
          const result = await queryWithTenantContext(companyId, query, values);
          ...
          if (prefer.includes('count=exact')) {
            ...
            const countResult = await queryWithTenantContext(companyId, countQuery, values);
