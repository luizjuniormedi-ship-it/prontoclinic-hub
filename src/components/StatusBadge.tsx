...
import { getAppointmentStatusLabel, getAppointmentTypeLabel, getPaymentStatusLabel, getReturnStatusLabel, getTherapyPackageStatusLabel } from "@/utils/formatters";
...
export function AppointmentStatusBadge({ status }: { status: AppointmentStatus }) {
  return (
    <Badge variant="outline" className={`${appointmentStatusVariants[status]} border-0 font-medium`}>
...
export function AppointmentTypeBadge({ type }: { type: AppointmentType }) {
  return (
    <Badge variant="outline" className={`${appointmentTypeVariants[type]} border-0 font-medium`}>
...
export function ReturnStatusBadge({ status }: { status: ReturnStatus }) {
  return (
    <Badge variant="outline" className={`${returnStatusVariants[status]} border-0 font-medium`}>
...
export function TherapyPackageStatusBadge({ status }: { status: TherapyPackageStatus }) {
  return (
    <Badge variant="outline" className={`${therapyPackageStatusVariants[status]} border-0 font-medium`}>
      {getTherapyPackageStatusLabel(status)}
    </Badge>
  );
}
