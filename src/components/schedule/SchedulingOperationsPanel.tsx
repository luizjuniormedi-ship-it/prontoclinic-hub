...
import { ScheduleGridsPanel } from "@/components/schedule/ScheduleGridsPanel";
...
export function SchedulingOperationsPanel({
  professionals,
  specialties,
  appointmentTypes,
  services,
          ...
          <TabsContent value="availability" className="space-y-3 pt-2">
            <div className="grid gap-2 md:grid-cols-[minmax(220px,1fr)_160px_120px_auto]">
              <Select value={professionalId} onValueChange={setProfessionalId}>
                <SelectTrigger aria-label="Profissional para disponibilidade"><SelectValue placeholder="Profissional" /></SelectTrigger>
              ...
              <Input aria-label="Data para disponibilidade" type="date" value={slotDate} onChange={(event) => setSlotDate(event.target.value)} />
              <Select value={duration} onValueChange={setDuration}><SelectTrigger aria-label="Duração para disponibilidade"><SelectValue /></SelectTrigger><SelectContent>{[10, 15, 20, 30, 40, 60].map((value) => <SelectItem key={value} value={String(value)}>{value} min</SelectItem>)}</SelectContent></Select>
          ...
          <TabsContent value="waitlist" className="space-y-3 pt-2">
            ...
            {waitlist.length === 0 ? <p className="text-sm text-muted-foreground">Nenhum paciente aguardando horário.</p> : waitlist.map((entry) => (
              <div
                key={entry.id}
                role="article"
                aria-label={`Espera de ${entry.patient_name || `Paciente ${entry.patient_id}`}`}
...
function WaitlistDialog({ open, onOpenChange, professionals, specialties, appointmentTypes, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; professionals: DbProfessional[]; specialties: DbSpecialty[]; appointmentTypes: DbAppointmentType[]; onCreated: () => Promise<void> }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Adicionar à lista de espera</DialogTitle><DialogDescription>Registre a preferência do paciente para localizar uma vaga compatível.</DialogDescription></DialogHeader><div className="grid gap-3"><div><Label htmlFor="waitlist-patient-search">Buscar paciente *</Label><Input id="waitlist-patient-search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Nome, CPF ou telefone" /></div>{patients.length > 0 && <Select value={patientId} onValueChange={setPatientId}><SelectTrigger aria-label="Paciente da lista de espera"><SelectValue placeholder="Selecione o paciente" /></SelectTrigger><SelectContent>{patients.map((p) => <SelectItem key={p.id} value={p.id}>{p.name} {p.cpf ? `- ${p.cpf}` : ""}</SelectItem>)}</SelectContent></Select>}<div className="grid grid-cols-1 md:grid-cols-3 gap-2"><Select value={specialtyId} onValueChange={setSpecialtyId}><SelectTrigger aria-label="Especialidade da lista de espera"><SelectValue placeholder="Especialidade" /></SelectTrigger><SelectContent><SelectItem value="all">Qualquer especialidade</SelectItem>{specialties.map((s) => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}</SelectContent></Select><Select value={professionalId} onValueChange={setProfessionalId}><SelectTrigger aria-label="Profissional da lista de espera"><SelectValue placeholder="Profissional" /></SelectTrigger><SelectContent><SelectItem value="all">Qualquer profissional</SelectItem>{professionals.map((p) => <SelectItem key={p.id} value={p.id}>{p.full_name}</SelectItem>)}</SelectContent></Select><Select value={typeId} onValueChange={setTypeId}><SelectTrigger aria-label="Tipo da lista de espera"><SelectValue placeholder="Tipo" /></SelectTrigger><SelectContent><SelectItem value="all">Qualquer tipo</SelectItem>{appointmentTypes.map((t) => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}</SelectContent></Select></div><div className="grid grid-cols-2 md:grid-cols-4 gap-2"><Input aria-label="Data inicial da lista de espera" type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} /><Input aria-label="Data final da lista de espera" type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} /><Select value={period} onValueChange={(v) => setPeriod(v as PreferredPeriod)}><SelectTrigger aria-label="Período preferido da lista de espera"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(periodLabels).map(([v, label]) => <SelectItem key={v} value={v}>{label}</SelectItem>)}</SelectContent></Select><Select value={priority} onValueChange={(v) => setPriority(v as WaitlistPriority)}><SelectTrigger aria-label="Prioridade da lista de espera"><SelectValue /></SelectTrigger><SelectContent>{Object.entries(priorityLabels).map(([v, label]) => <SelectItem key={v} value={v}>{label}</SelectItem>)}</SelectContent></Select></div><div><Label htmlFor="waitlist-reason">Motivo *</Label><Input id="waitlist-reason" value={reason} onChange={(e) => setReason(e.target.value)} /></div><Textarea aria-label="Observações da lista de espera" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Observações" /></div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button onClick={() => void submit()} disabled={saving}>{saving ? "Salvando..." : "Adicionar"}</Button></DialogFooter></DialogContent></Dialog>;
}
...
function BlockDialog({ open, onOpenChange, professionals, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; professionals: DbProfessional[]; onCreated: () => Promise<void> }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Novo bloqueio de agenda</DialogTitle><DialogDescription>Indisponibilize um período com motivo auditável para evitar novos agendamentos.</DialogDescription></DialogHeader><div className="space-y-3"><Select value={professionalId} onValueChange={setProfessionalId}><SelectTrigger aria-label="Profissional do bloqueio"><SelectValue placeholder="Profissional" /></SelectTrigger><SelectContent>{professionals.map((p) => <SelectItem key={p.id} value={p.id}>{p.full_name}</SelectItem>)}</SelectContent></Select><div className="grid grid-cols-2 gap-2"><div><Label htmlFor="block-starts-at">Início</Label><Input id="block-starts-at" type="datetime-local" value={startsAt} onChange={(e) => setStartsAt(e.target.value)} /></div><div><Label htmlFor="block-ends-at">Fim</Label><Input id="block-ends-at" type="datetime-local" value={endsAt} onChange={(e) => setEndsAt(e.target.value)} /></div></div><Select value={type} onValueChange={setType}><SelectTrigger aria-label="Tipo do bloqueio"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="operational">Operacional</SelectItem><SelectItem value="leave">Afastamento</SelectItem><SelectItem value="vacation">Férias</SelectItem><SelectItem value="meeting">Reunião</SelectItem><SelectItem value="emergency">Emergência</SelectItem></SelectContent></Select><Textarea aria-label="Motivo do bloqueio" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Motivo obrigatório" /></div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button onClick={() => void submit()} disabled={saving}>{saving ? "Salvando..." : "Bloquear"}</Button></DialogFooter></DialogContent></Dialog>;
}
...
function ConvertDialog({ entry, onOpenChange, onConverted }: { entry: WaitlistEntry | null; onOpenChange: (open: boolean) => void; onConverted: () => Promise<void> }) {
  ...
  return <Dialog open={Boolean(entry)} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Converter espera em agendamento</DialogTitle><DialogDescription>Confirme a data e o horário antes de reservar a vaga para o paciente.</DialogDescription></DialogHeader><p className="text-sm">{entry?.patient_name} · {entry?.professional_name || "Profissional não definido"}</p><div className="grid grid-cols-3 gap-2"><Input type="date" value={date} onChange={(e) => setDate(e.target.value)} /><Input type="time" value={start} onChange={(e) => setStart(e.target.value)} /><Input type="time" value={end} onChange={(e) => setEnd(e.target.value)} /></div><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button onClick={() => void submit()} disabled={saving || !entry?.professional_id}>{saving ? "Convertendo..." : "Confirmar"}</Button></DialogFooter></DialogContent></Dialog>;
}
