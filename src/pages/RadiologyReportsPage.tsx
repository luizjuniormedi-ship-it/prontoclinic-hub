...
export default function RadiologyReportsPage() {
  ...
  const confirmSign = async () => {
    ...
    setBusy(true);
    try {
      const signed = await reportsService.signAndRelease(signDialog.id);
      toast({ title: "Laudo assinado e liberado", description: `Por ${signed.signed_by_name}` });
      setSignDialog(null); load();
  ...
  const canSign = (s: ReportStatus) => ["em_revisao", "aguardando_assinatura"].includes(s);
                ...
                <TableRow key={r.id} className={r.has_critical_finding ? "bg-destructive/5" : ""}>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      ...
                      {canSign(r.status) && <Button size="sm" variant="ghost" className="h-6 text-[10px]" onClick={() => setSignDialog(r)} title="Assinar"><CheckCircle2 className="h-3 w-3 text-success" /></Button>}
      ...
      <Dialog open={!!signDialog} onOpenChange={(v) => !v && setSignDialog(null)}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3 py-2">
            ...
            <p className="text-sm text-muted-foreground">A identidade e o CRM serão obtidos do profissional vinculado à sua sessão. Sem CRM cadastrado, a assinatura será recusada.</p>
            <p className="text-[10px] text-muted-foreground">Assinatura e liberação ocorrem em uma única transação e tornam o laudo somente-leitura.</p>
          </div>
