...
export function isBillingReadyForSubmission(account: Pick<BillingAccount, "status" | "has_pending_issues">): boolean {
  return account.status === "pronta_envio" || (account.status === "aberta" && !account.has_pending_issues);
}
...
export const billingAccountsService = {
  ...
  async stats(): Promise<{ total: number; abertas: number; prontas: number; comPendencia: number; enviadas: number; pagas: number }> {
    ...
    return {
      ...
      prontas: all.filter(isBillingReadyForSubmission).length,
