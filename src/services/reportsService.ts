...
export const reportsService = {
  ...
  /** Assina o laudo: grava assinatura, muda status para assinado, registra em report_signatures. */
  async sign(id: string, signerName: string, signerCrm: string): Promise<void> {
    const now = new Date().toISOString();
    const { error } = await supabase.from("reports").update({
      status: "assinado", signed_by_name: signerName, signed_by_crm: signerCrm, signed_at: now,
    }).eq("id", id);
    if (error) throw new Error(error.message);
    await supabase.from("report_signatures").insert({
      report_id: id, signer_name: signerName, signer_crm: signerCrm, signer_role: "laudador",
    });
  },

  async release(id: string): Promise<void> {
    await reportsService.update(id, { status: "liberado", released_at: new Date().toISOString() });
  },
  ...
  async rectify(report: Report, motivo: string): Promise<void> {
    await supabase.from("report_versions").insert({
      ...
      technique: report.technique, findings: report.findings,
      conclusion: report.conclusion, recommendation: report.recommendation,
      motivo_retificacao: motivo,
    });
    const { error } = await supabase.from("reports").update({
      status: "em_digitacao", is_rectified: true, version: (report.version || 1) + 1,
    }).eq("id", report.id);
    if (error) throw new Error(error.message);
  },
  ...
  async logDelivery(id: string, canal: string, destinatario: string): Promise<void> {
    await supabase.from("reports").update({ status: "entregue", delivered_at: new Date().toISOString() }).eq("id", id);
    await supabase.from("report_delivery_logs").insert({ report_id: id, canal, destinatario });
  },
