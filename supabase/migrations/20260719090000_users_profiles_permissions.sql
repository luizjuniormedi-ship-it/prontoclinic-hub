...
GRANT EXECUTE ON FUNCTION public.get_my_company_id() TO authenticated, app_prontomedic;
...
-- Instalações legadas já possuem public.user_permissions para flags históricas.
-- Mantemos essas colunas e acrescentamos o contrato canônico de overrides.
ALTER TABLE public.user_permissions
  ADD COLUMN IF NOT EXISTS company_id UUID REFERENCES public.companies(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS permission_id BIGINT REFERENCES public.permissions(id) ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS effect VARCHAR(10) DEFAULT 'grant',
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid = 'public.user_permissions'::regclass
      AND conname = 'user_permissions_effect_check'
  ) THEN
    ALTER TABLE public.user_permissions
      ADD CONSTRAINT user_permissions_effect_check
      CHECK (effect IS NULL OR effect IN ('grant', 'deny'));
  END IF;
END
$$;

CREATE UNIQUE INDEX IF NOT EXISTS uq_user_permissions_override_scope
...
SELECT r.id, modules.module,
  ...
  OR (r.name = 'callcenter' AND modules.module IN ('dashboard','patients','schedule','callcenter'))
  OR (r.name IN ('medico','medico_laudador') AND modules.module IN ('dashboard','patients','schedule','records','encounters','attendance','dicom'))
  OR (r.name IN ('enfermagem','tecnico_enfermagem') AND modules.module IN ('dashboard','patients','reception','nursing'))
  OR (r.name IN ('financeiro','auditor') AND modules.module IN ('dashboard','billing','financial')),
  r.name = 'admin'
  OR (r.name IN ('recepcao','supervisor_recepcao','callcenter') AND modules.module IN ('patients','schedule','reception','callcenter')),
