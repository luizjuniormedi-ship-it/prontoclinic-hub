-- Derive reception workflow scope from the canonical precheck contract.
...
BEGIN
  ...
  IF v_company IS NULL THEN
    RAISE EXCEPTION 'Contexto de empresa ausente';
  END IF;
  ...
  IF NULLIF(v_precheck->>'appointment_id', '')::BIGINT
    ...
    RAISE EXCEPTION 'Contexto do agendamento inconsistente';
  END IF;
  IF v_unit_id IS NULL OR v_patient_id IS NULL THEN
    RAISE EXCEPTION
