...
BEGIN
  IF to_regprocedure(
       'public.m23_acknowledge_critical_alert_secure(bigint,text,text)'
     ) IS NULL THEN
    RAISE EXCEPTION 'M23 critical alert RPC is missing';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_roles WHERE rolname = 'prontomedic_lis_rpc_owner'
  ) THEN
    RAISE EXCEPTION 'M23 LIS RPC owner is missing';
  END IF;
END
$prerequisites$;
...
BEGIN
  ...
  IF p_reference_id IS NULL THEN
    INSERT INTO public.exames_lab_valor_referencia (
      company_id, cd_exame, ds_parametro, vl_minimo, vl_maximo,
      ds_unidade, cd_sexo, nr_idade_min, nr_idade_max, lg_ativo
    )
    VALUES (
      v_company, p_exam_id, BTRIM(p_payload->>'parameter'),
      v_minimum, v_maximum, NULLIF(BTRIM(p_payload->>'unit'), ''),
      v_sex, v_minimum_age, v_maximum_age,
      coalesce((p_payload->>'active')::BOOLEAN, TRUE)
    )
    RETURNING id INTO v_id;
  ELSE
    UPDATE public.exames_lab_valor_referencia AS reference_range
    SET cd_exame = p_exam_id,
        ...
        lg_ativo = coalesce(
          ...
          reference_range.lg_ativo
        )
    WHERE reference_range.id = p_reference_id
      AND reference_range.company_id = v_company
      AND EXISTS (
        SELECT 1 FROM public.exames_lab_catalogo AS catalog
        WHERE catalog.id = p_exam_id
          AND catalog.company_id = v_company
      )
    RETURNING reference_range.id INTO v_id;
...
-- PostgreSQL does not allow CREATE OR REPLACE to remove an existing argument
-- default. The restricted DROP fails closed if an unexpected dependency exists.
DROP FUNCTION public.m23_acknowledge_critical_alert_secure(BIGINT, TEXT, TEXT);
...
GRANT USAGE ON SCHEMA m23_private TO prontomedic_lis_rpc_owner;
GRANT EXECUTE ON FUNCTION m23_private.acknowledge_critical_alert(BIGINT, TEXT, TEXT)
